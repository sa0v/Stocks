...
simulated_prices[1] <- as.numeric(data[length(data)])
...
# Apply EMA to smooth the simulated prices
smoothed_simulated_prices <- EMA(simulated_prices, n = 10)  # n is the window size for smoothing
...
predicted_data <- data.frame(
  Date = forecast_dates,
  Price = smoothed_simulated_prices,
  Type = "Predicted"
)
...
# Step 1: Calculate Strategy Returns (for the next 30 days)
# Replace simulated_prices_30 with smoothed_simulated_prices
predicted_returns_30 <- diff(c(0, smoothed_simulated_prices))  # Predicted returns for 30 days
actual_returns_30 <- diff(c(0, smoothed_simulated_prices))  # Actual returns from predicted prices

# Generate Buy, Hold, Sell strategy based on price change thresholds
threshold_buy <- 0.01  # Buy if price increases by 1% compared to the previous day
threshold_sell <- -0.01  # Sell if price decreases by 1%
...
# Step 3: Calculate Directional Accuracy (for the next 30 days)
# Generate actual signals (Buy if price increases, Sell if price decreases, Hold otherwise)
actual_signals_30 <- ifelse(diff(c(0, smoothed_simulated_prices)) > 0, "Buy", 
                            ifelse(diff(c(0, smoothed_simulated_prices)) < 0, "Sell", "Hold"))

# Generate predicted signals (based on thresholds)
predicted_signals_30 <- ifelse(diff(c(0, smoothed_simulated_prices)) > threshold_buy, "Buy", 
                               ifelse(diff(c(0, smoothed_simulated_prices)) < threshold_sell, "Sell", "Hold"))
...
# Step 4: Plot the Predicted Prices with Buy/Hold/Sell Signals for the next 30 days
...
predicted_30_days_df <- data.frame(
  ...
  PredictedPrice = smoothed_simulated_prices[1:30],  # Use the first 30 smoothed prices
...
# Plot using ggplot2 with Buy/Hold/Sell signals
ggplot(predicted_30_days_df, aes(x = Date, y = PredictedPrice)) +
  ...
  geom_point(aes(color = Signal), size = 3) +  # Plot points for Buy/Hold/Sell signals
  scale_color_manual(values = c("Buy" = "green", "Hold" = "yellow", "Sell" = "red")) +
  labs(
