...
predicted_returns_30 <- diff(c(0, simulated_prices))  # Predicted returns for 30 days
actual_returns_30 <- diff(c(0, simulated_prices))  # Actual returns from predicted prices
...
actual_signals_30 <- ifelse(diff(c(0, simulated_prices)) > 0, "Buy", 
                            ifelse(diff(c(0, simulated_prices)) < 0, "Sell", "Hold"))

# Generate predicted signals (based on thresholds)
predicted_signals_30 <- ifelse(diff(c(0, simulated_prices)) > threshold_buy, "Buy", 
                               ifelse(diff(c(0, simulated_prices)) < threshold_sell, "Sell", "Hold"))
...
predicted_30_days_df <- data.frame(
  ...
  PredictedPrice = simulated_prices[1:30],  # Use the first 30 smoothed prices
